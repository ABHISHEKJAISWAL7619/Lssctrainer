...
const Addperformance = ({ reportId }) => {
  ...
  const fetchBatches = async () => {
    ...
    // ✅ Auto-select latest batch if user hasn't selected manually
    if (data.length > 0 && selectedBatch === "Select Batch") {
      const latestBatch = data[data.length - 1];
      setSelectedBatch(latestBatch.name);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!batchMap[selectedBatch]) {
      await fetchBatches(); // Re-fetch just in case it's out of sync
      if (!batchMap[selectedBatch]) {
        setError("Invalid batch selected");
        return;
      }
    ...
    const requiredFields = [
      { field: "timeOfReporting", msg: "Please enter Time of Reporting" },
      { field: "sessionTime", msg: "Please enter Session Time" },
      { field: "totalBatchStrength", msg: "Please enter Total Batch Strength" },
      {
        field: "noOfStudentsAttended",
        msg: "Please enter No of Students Attended",
      },
      {
        field: "practicalSkillRating",
        msg: "Please enter Practical Skill Rating",
      },
      {
        field: "theoryUnderstandingRating",
        msg: "Please enter Theory Understanding Rating",
      },
      { field: "behavioralFeedback", msg: "Please enter Behavioral Feedback" },
      {
        field: "overallPassStatus",
        msg: "Please enter Overall Pass/Fail Status",
      },
      {
        field: "certificationStatus",
        msg: "Please enter Certification Status",
      },
    ];
